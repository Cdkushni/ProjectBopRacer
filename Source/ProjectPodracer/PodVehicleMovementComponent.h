...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PodMovement|Drift")
	float DriftMomentumDecayRate; // How quickly the lateral momentum decays during drift (lower = longer slide)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PodMovement|Drift")
	float DriftLateralContributionScale; // Scales how much CurrentLateralMomentum adds to OutVelocity
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PodMovement|Drift")
	float DriftLateralMomentumMax; // Maximum magnitude for CurrentLateralMomentum
...
protected:
	...
	// Sub-functions for modular movement
	void HandleDriftState(bool InIsDrifting, float DeltaTime, FVector& OutVelocity, const FVector& ForwardVector);
	void ApplyDamping(float DeltaTime, bool InIsDrifting, bool bGrounded, FVector& OutVelocity, const FVector& GroundNormal);
	void ApplyLateralFriction(float DeltaTime, bool InIsDrifting, bool bGrounded, FVector& OutVelocity, const FVector& RightVector);
	FVector CapSpeed(const FVector& InVelocity, float Max, bool bGrounded) const;
	void ApplyAcceleration(float DeltaTime, float InForwardInput, bool InBoosting, bool InBraking, float ControlMul, const FVector& Forward, FVector& OutVelocity);
	void ApplySteering(float DeltaTime, float InTurnInput, bool InDrifting, bool bGrounded, float EffectiveMaxSpeed, FRotator& OutRotation, float& OutAngularYawVelocity);

	FVector GetGroundNormal(FHitResult& OutHit) const;
